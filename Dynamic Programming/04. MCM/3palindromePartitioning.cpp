...
int ppRecursion(string s, int i, int j){
    if(i>=j)    return 0;
    if(isPalindrome(s,i,j)) return 0;

    int mn=INT_MAX;
    loopk(i,j){
        int temp = 1+ppRecursion(s,i,k)+ppRecursion(s,k+1,j);
        if(temp<mn) mn=temp;
    }
    return mn;
}
...
int main(){
    ...
    cout<<"Minimum number of partions to be done to get the substrings as a palindrome are(using recurion) = "<<ppRecursion(s,0,s.length()-1)<<endl;
    return 0;
}